...
foot = [0 0 0; 0.6 0.1 0.06; 0.9 0.2 -0.03; 1.2 0.3 0.12; 1.5 0.4 0.03; 1.8 0.5 0.18; 2.1 0.6 0.09; 2.4 0.6 0.15; 100 0 0]; %最後は終了フラグ
forward_period = 1.0;                   % 予見制御の時間(s) 一般的には1ほど
calculate_period = 4.0;                 % 歩行パターンを生成する期間(s)
dt = 0.01;                              % サンプリングタイム (s)
zh = 0.27;                              % 重心位置 (m) 床からの高さ
g  = 9.8;                               % 重力加速度 (m/s^2)
...
Q = zeros(4);
Q(1) = 10^3;                            % ★★★最適制御の重み係数　大：ZMPが理想に近づく 要変更
H = 1;                                  % 同じく重み係数　大：入力が少なくなる．要変更

P = dare(Phai, G, Q, H);                % 離散時間系でのリカッチ方程式の解
F = -inv(H+G'*P*G)*G'*P*Phai;                                % ★★★フィードバックゲイン 式(2.13)　Pに基づいた式
